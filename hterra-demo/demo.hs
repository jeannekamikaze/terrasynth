...
fBmPerlin seed cs hu l o w h file =
          let ps  = perms seed 256
              ...
              params = fBmParams basisParams hu l
...
data NoiseType = Perlin | FbmCPU | FbmGPU | Cells deriving (Data, Typeable, Show)
...
defaultArgs = cmdArgsMode $ Demo
            { noise = Perlin   &= name "t" &= typ "Perlin | FbmCPU | FbmGPU | Cells"
...
main = do
     ...
     case noise of
          ...
          FbmGPU -> fBmPerlin' s cs hu lacu noct w h file
          FbmCPU -> fBmPerlin  s cs hu lacu noct w h file
